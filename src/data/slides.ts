...
export const slides: Slide[] = unsplashPhotos.map(({ id, width, height }) => {
  ...
  return {
    src: unsplashLink(id, scaledWidth, scaledHeight),
    width: scaledWidth,
    height: scaledHeight,
    srcSet: breakpoints.map((bpWidth) => {
      const bpHeight = Math.round((scaledHeight / scaledWidth) * bpWidth);
      return {
        src: unsplashLink(id, bpWidth, bpHeight),
        width: bpWidth,
        height: bpHeight,
      };
    }),
